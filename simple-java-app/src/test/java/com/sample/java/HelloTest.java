package com.sample.java;

import org.junit.Test;

import static org.hamcrest.CoreMatchers.*;
import static org.junit.Assert.*;

/**
 * Unit test for Hello.
 * <p/>
 * A unit test aims to test all code and code paths of a specific class.
*/ 
public class HelloTest {

    @Test
    public void testSayHello() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertNotEquals(hello, "Hello");
        assertEquals("1", "1");
                
    }
    
    @Test
    public void testMickey() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Mickey", "Mickey");
                
    }
    
    @Test
    public void testMickey1() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Donald", "Donald");
                
    }
    
    @Test
    public void testMickey2() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Minnie", "Minnie");
                
    }
    
    @Test
    public void testMickey3() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Donald", "Donald");
                
    }

    @Test
    public void testMickey4() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Elza", "Elza");
                
    }

     @Test
    public void testMickey5() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Elza", "Elza");
                
    }

         @Test
    public void testMickey6() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Elza", "Elza");
                
    }

    @Test
    public void testMickey7() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Elza", "Elza");
                
    }

        @Test
    public void testMickey8() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Elza", "Elza");
                
    }

    
        @Test
    public void testMickey9() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Hugo", "Hugo");
                
    }
    
            @Test
    public void testMickeyx() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Karen", "Karen");
                
    }
    
     @Test
    public void testMickeyy() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Karen", "Karen");
                
    }
    
         @Test
    public void testMickeyz() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Karen", "Karen");
                
    }
    
             @Test
    public void testMickeyzz() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Karen", "Karen");
                
    }
    
                 @Test
    public void testMickeyboc() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Karen", "Karen");
                
    }
        
                 @Test
    public void testMickeybupa() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Karen", "Karen");
                
    }
    
                     @Test
    public void testMickeychiu() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Amy", "Amy");
                
    }
    
    
                         @Test
    public void testMickeyDona() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Dona", "Dona");
                
    }
    
                             @Test
    public void testTechnet() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Mic", "Mic");
                
    }
    
                                 @Test
    public void testMacauUST() {
        Hello hi = new Hello();
        String hello = hi.sayHello();

        assertEquals("Mic", "Mic");
                
    }
    
}
